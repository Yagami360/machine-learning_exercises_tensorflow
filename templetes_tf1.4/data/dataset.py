...
def load_dataset(
    ...
    if( use_tfrecord ):
        ...
        ds_train_s = ds_train_s.map(preprocessing, num_parallel_calls=1)
        ds_valid_s = ds_valid_s.map(preprocessing, num_parallel_calls=1)
        ds_train_t = ds_train_t.map(preprocessing, num_parallel_calls=1)
        ds_valid_t = ds_valid_t.map(preprocessing, num_parallel_calls=1)
        ...
        if( use_prefeatch ):
            ...
            ds_valid = ds_valid.prefetch(buffer_size=1)
...
#@tf.function
def preprocessing( image_path, image_height = 128, image_width = 128, offset = True ):
    """
    前処理スクリプト
    """
    ...
    if( offset ):
        preprocess_op = preprocess_op * 2.0 - 1.0
...
class Dataset(object):
    ...
    def data_augument( self, image_tsr ):
        return image_tsr

    def get_batch_data( self, sess ):
        image_s, image_t = sess.run(self.batch_op)
