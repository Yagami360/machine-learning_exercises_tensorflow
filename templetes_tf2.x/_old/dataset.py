...
def write_tfrecord( image_s, image_t ):
    """
    TFRecordは、tf.train.Example を１つのレコードの単位として書き込む
    tf.train.Example は数値や画像などの固定長のリストを扱い、各レコードの値を tf.train.Feature で指定する
    ...
    # tf.train.Example は数値や画像などの固定長のリストを扱い、各レコードの値を tf.train.Feature で指定する
    feature = {
        "image_s" : tf.train.Feature( bytes_list=tf.train.BytesList(value=[image_s_str]) ),
        "image_t" : tf.train.Feature( bytes_list=tf.train.BytesList(value=[image_t_str]) ),
...
def load_dataset(
    ...
    if( use_tfrecord ):
        ...
        print( "ds_train_s : ", ds_train_s )
        """
        for item in ds_train_s:
            print( item )
        """
        for data in ds_train_s.take(1):
